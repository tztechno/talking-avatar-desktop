...
import { guessVisemeFromBands, timelineViseme } from "../src/lipsync/audio-lipsync";
...
describe("audio lip-sync helpers", () => {
  ...
  it("spreads a timeline across a chunk", () => {
    expect(timelineViseme(["A", "I", "U"], 0, 3)).toBe("A");
    expect(timelineViseme(["A", "I", "U"], 1.5, 3)).toBe("I");
    expect(timelineViseme(["A", "I", "U"], 3, 3)).toBe("U");
    expect(timelineViseme([], 1, 3)).toBeUndefined();
  });
});
